"""Public Data Function App (Azure Functions Python v2 model) — public edition.

Serves read-only product catalog, inventory, and order data over HTTP. Unlike the
private (APIM-fronted) and network-injection editions of this project, this app is
deployed with **public network access enabled** and an interactive Swagger UI, so the
APIs can be browsed and called directly from the public internet.

It shares the dedicated B1 Linux App Service plan ``asp-fdryvnetgw-data-eastus`` with
the private edition (a dedicated plan can host multiple apps), but keeps its own
storage account and system-assigned managed identity.

All routes are anonymous; this app intentionally exposes sample, non-sensitive data.
"""
...
def health(req: func.HttpRequest) -> func.HttpResponse:
    return _json_response(
        {
            "status": "healthy",
            "service": "foundry-data-function-public-api",
...
def openapi(req: func.HttpRequest) -> func.HttpResponse:
    ...
    # downstream importers resolve the correct public base.
...
<head>
  <meta charset="UTF-8" />
  <title>Foundry Data Function API (Public) - Swagger UI</title>
