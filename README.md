...
- [Sample Teams Prompt: End-to-End API Call Trace](#sample-teams-prompt-end-to-end-api-call-trace)
...
## Sample Teams Prompt: End-to-End API Call Trace

This section walks through every API call that happens when a user sends a single prompt to a Foundry agent from Microsoft Teams, and shows how the correct agent is selected when the Foundry project hosts more than one (for example `Tax-PDF-Forms-Agent` and `Eng-Design-PPT-Agent`).

**Sample user prompt (typed in the Teams compose box):**

> *"What is the standard deduction reported on Form 1040 for the current tax year?"*

### Call sequence overview

```
Teams client
   |  (1) Bot Framework activity
   v
Azure Bot Service  --->  Bot Function App  (POST /api/messages)
                              |
                              |  (2) infer use_case from prompt text
                              |      payload = { "prompt": "...", "use_case": "tax_pdf_forms" }
                              v
                         APIM AI Gateway  (POST /foundry-privatevnet-app/chat)
                              |
                              |  (3) APIM policy: auth + routing
                              v
                         FastAPI backend (api/server.py  POST /api/chat)
                              |
                              |  (4) map use_case -> agent_name -> assistant_id
                              v
                         APIM Agents Gateway (path: /foundry-agents/...)
                              |
                              |  (5) GET  /assistants                          -> resolve assistant_id
                              |  (6) POST /threads                              -> thread_id
                              |  (7) POST /threads/{thread_id}/messages         -> add user prompt
                              |  (8) POST /threads/{thread_id}/runs             -> { assistant_id }
                              |  (9) GET  /threads/{thread_id}/runs/{run_id}    -> poll status
                              |  (10) GET /threads/{thread_id}/messages         -> assistant reply
                              v
                         Private Foundry project (services.ai.azure.com)
```

### How the agent is identified across Foundry's multiple hosted agents

Foundry projects can host many agents. This solution selects the right one by chaining three identifiers:

| Layer | Identifier | Where it lives | Example |
|-------|-----------|----------------|---------|
| Teams / Bot | `use_case` (logical key) | Inferred in [bot-function/bot_app.py](bot-function/bot_app.py#L76) by `_infer_use_case` from prompt text, or pinned via the `USE_CASE` env var | `tax_pdf_forms` |
| Backend API | `agent_name` (display name) | Mapped in [config/agent_config.json](config/agent_config.json) and resolved in [api/server.py](api/server.py#L255) | `Tax-PDF-Forms-Agent` |
| Foundry Agents API | `assistant_id` (Foundry-issued GUID) | Returned by `GET /assistants`, then sent on `POST /threads/{id}/runs` as `assistant_id` | `asst_abc123...` |

The `use_case` field in the `/chat` request body is the canonical agent selector across the whole pipeline. The backend translates it to the Foundry `assistant_id` only at the last hop, so callers (Teams, Web Chat, Pipeline UI) never need to know Foundry's GUIDs.

### Step-by-step API calls and payloads

#### (1) Teams -> Bot Function App

Teams posts a Bot Framework `Activity` to the Function App. Identifying fields are highlighted.

`POST https://<bot-function-app>.azurewebsites.net/api/messages`
...
```json
{
  ...
  "id": "1715600000001",
  "channelId": "msteams",
  "from": { "id": "29:1abc...", "name": "Jane Doe", "aadObjectId": "..." },
  "recipient": { "id": "28:<bot-app-id>", "name": "Tax PDF Forms Agent" },
  "conversation": { "conversationType": "personal", "id": "a:1xyz..." },
  "text": "What is the standard deduction reported on Form 1040 for the current tax year?",
  "serviceUrl": "https://smba.trafficmanager.net/amer/"
}
```

Response: `200 OK` (empty body; the bot replies asynchronously via the Bot Framework adapter).

#### (2) Bot Function App -> APIM AI Gateway

The bot extracts the prompt, calls `_infer_use_case` to pick `tax_pdf_forms` vs `eng_design_ppt`, then POSTs to APIM.

`POST https://<apim-name>.azure-api.net/foundry-privatevnet-app/chat`

Headers:

```
Content-Type: application/json
Ocp-Apim-Subscription-Key: <key>   # only if the API requires a subscription
```
...
```json
{
  "prompt": "What is the standard deduction reported on Form 1040 for the current tax year?",
  "use_case": "tax_pdf_forms"
}
```

#### (3) APIM AI Gateway -> FastAPI backend

APIM applies its inbound policy (managed-identity auth, request rewrite, routing) and forwards to the backend App Service. The backend handler is `chat()` in [api/server.py](api/server.py#L450), which calls `_invoke_agent(prompt, use_case)`.

#### (4) Backend resolves `use_case` -> `agent_name`

`_require_use_case("tax_pdf_forms")` reads [config/agent_config.json](config/agent_config.json) and yields:

```json
{
  "agent_name": "Tax-PDF-Forms-Agent",
  "model_deployment": "gpt-4.1"
}
```

All subsequent calls in steps 5-10 go through APIM's second API (`/foundry-agents/...`), which proxies to the private Foundry endpoint `https://<foundry-account>.services.ai.azure.com`.

#### (5) Resolve `assistant_id` from agent name

`GET https://<apim-name>.azure-api.net/foundry-agents/api/projects/<project>/assistants?api-version=2025-05-01`
...
{
  "data": [
    { "id": "asst_8K2pWv...", "name": "Tax-PDF-Forms-Agent", "model": "gpt-4.1" },
    { "id": "asst_4QnRf3...", "name": "Eng-Design-PPT-Agent", "model": "gpt-4.1" }
  ]
}
```

The backend selects `asst_8K2pWv...` because its `name` matches `Tax-PDF-Forms-Agent`. **This is the moment a single Foundry project's many agents are disambiguated.**
...
```json
{ "id": "thread_01HQ...", "object": "thread", "created_at": 1715600001 }
```

#### (7) Add the user prompt as a message

`POST .../threads/thread_01HQ.../messages?api-version=2025-05-01`

Body:

```json
{ "role": "user", "content": "What is the standard deduction reported on Form 1040 for the current tax year?" }
```

Response:

```json
{ "id": "msg_01HR...", "object": "thread.message", "thread_id": "thread_01HQ...", "role": "user" }
```

#### (8) Start a run, binding the thread to the chosen assistant

`POST .../threads/thread_01HQ.../runs?api-version=2025-05-01`

Body (this is where the agent identity is bound to the conversation):

```json
{ "assistant_id": "asst_8K2pWv..." }
```
...
`GET .../threads/thread_01HQ.../runs/run_01HS...?api-version=2025-05-01`

Response cycles through `queued` -> `in_progress` -> `completed`:

```json
{ "id": "run_01HS...", "status": "completed", "assistant_id": "asst_8K2pWv..." }
```

#### (10) Fetch the assistant's reply

`GET .../threads/thread_01HQ.../messages?api-version=2025-05-01`
      ...
      "content": [
        {
          "type": "text",
          "text": {
            "value": "For the current tax year, Form 1040 reports a standard deduction of ...",
            "annotations": [
              { "type": "file_citation", "file_citation": { "file_id": "assistant-...", "quote": "..." } }
            ]
          }
        }
      ]
    },
    { "id": "msg_01HR...", "role": "user", "content": [ ... ] }
  ]
}
```

#### Final response returned to Teams

The backend flattens the assistant message and returns:

`200 OK` from `POST /chat`

```json
{
  "prompt": "What is the standard deduction reported on Form 1040 for the current tax year?",
  "use_case": "tax_pdf_forms",
  "response": "For the current tax year, Form 1040 reports a standard deduction of ...",
  "sources": ["f1040.pdf"],
  "duration_ms": 4821,
  "attempts": 1,
  "ok": true,
...
The bot reads `response`, then sends it back as a Bot Framework `message` activity that Teams renders as the agent's reply in the chat.

### Quick reference: agent selection summary

- `use_case` is the only agent selector clients send. Add a new agent by adding a new `use_case` entry in [config/agent_config.json](config/agent_config.json) and (for Teams) shipping an additional agent package under [Agent-Packages/](Agent-Packages/).
- The backend never hard-codes Foundry GUIDs. `_resolve_assistant_id` looks up the GUID by name on every cold start, so re-creating an agent in Foundry does not require redeploying the API.
- All 10 calls above traverse APIM, so policy enforcement (auth, rate limits, token-limit, content safety, logging) applies uniformly regardless of which Foundry agent is targeted.
