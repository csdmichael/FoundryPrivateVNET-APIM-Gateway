"""Private Data Function App (Azure Functions Python v2 model).

Serves read-only product catalog, inventory, and order data over HTTP. The app
is deployed with public network access disabled and is reachable only through a
private endpoint inside the VNet; the private APIM AI gateway fronts these APIs
and a Foundry agent consumes them via an OpenAPI tool.

All routes are anonymous at the function layer because access control is enforced
upstream by APIM (subscription/policy) and the private network boundary.
"""
...
def health(req: func.HttpRequest) -> func.HttpResponse:
    return _json_response(
        {
            "status": "healthy",
            "service": "foundry-data-function-api",
...
def openapi(req: func.HttpRequest) -> func.HttpResponse:
    ...
    # Rewrite the server URL to the caller's host so Swagger UI "Try it out"
    # and downstream importers resolve the correct base (function host or APIM path).
...
<head>
  <meta charset="UTF-8" />
  <title>Foundry Data Function API - Swagger UI</title>
